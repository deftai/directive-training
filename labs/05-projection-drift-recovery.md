...
| Directive baseline | `@deftai/directive@0.119.5` and core/content/types 0.119.5; [source baseline](../references/SOURCE-BASELINE.md) |
...
**Pass:** the helper prints `OK: CLI/core/content/types 0.119.5`; the explicit local CLI
reports core 0.119.5. The fixture pins the CLI and overrides its core, content, and types
packages to 0.119.5. Package-count and npm notices can vary; they are not acceptance evidence.
...
The following 0.119.5 help probes have side effects:
...
| --- | --- | --- |
| Start | Guard, pin proof, `lab-05-start`, status | Correct temp root, 0.119.5 graph, no remote, clean tracked state |
...
| `verify-pin` | 0 | CLI/core/content/types all 0.119.5 | O5.3 |
...
| Version differs | Explicit local version and `verify-pin` | Fresh reset with the exact fixture pin; do not use global CLI fallback | Both match 0.119.5 |
...
> I completed lab-05-projection-drift-recovery against Directive 0.119.5 on [actual OS and
