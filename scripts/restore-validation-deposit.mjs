...
export function restoreValidationDeposit(projectRoot, manifestPath) {
  ...
  assert.ok(pkg?.private === true && pkg.devDependencies?.["@deftai/directive"] === pin, "expected private project with exact 0.119.5 pin");
  const generation = JSON.parse(readFileSync(safePath(root, ".deft/GENERATION.json"), "utf8"));
  assert.ok(generation?.schemaVersion === 1 && Number.isInteger(generation.generation) && generation.generation > 0 && generation.contentVersion === pin && ["payload", "templates", "skills", "docs"].every((key) => generation.surfaces?.[key] === pin) && generation.surfaces?.version === "v" + pin, "expected existing 0.119.5 GENERATION record; do not rewrite it");
  ...
  assert.ok(manifest?.version === pin && Array.isArray(manifest.files), "expected 0.119.5 headless manifest");
  const nodes = new Map();
