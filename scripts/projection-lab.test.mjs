...
test("verifyPin rejects missing and changed package graphs", () => {
  ...
  writeFileSync(path, '{"version":"0.119.3"}');
  assert.throws(() => verifyPin(root), /0.119.2/);
  writeFileSync(path, '{"version":"0.119.2"}');
  archiveAttempt(root);
});
...
test("command interface validates operands and routes guarded fixture actions", () => {
  ...
  try {
    ...
    fakeGraph(root);
    assert.match(main(["verify-pin"]), /CLI\/core\/content\/types 0.119.2/);
    writeFileSync(join(root, "package-lock.json"), "{}\n");
