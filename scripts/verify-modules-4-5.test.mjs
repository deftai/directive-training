...
test("rejects a missing current platform marker set", (t) => {
  const files = fixture(t);
  files.change("references/SOURCE-BASELINE.md", (body) => body.replaceAll("teaching-platform-proof:", "platform-proof:"));
  assert.throws(() => verifyModules45(files.root), /current proof marker/);
});
